...
else: df_urls = pd.read_excel(r"G:\My Drive\Affiliate Marketing\urls_new_courses\2021\3-March\selected_courses_20210306.xlsx", "PEDIDO")
...
"""
    ...
    try:
        ...
        try:
            ...
            try:
                type_affiliation = list_text.index('enviado!')
                tipo_afiliacao = "AFILIAÇÃO PEDIDA"
            except:
    ...
    if tipo_afiliacao == 'ABERTA':
        ...
        preco_max = tratar_preco_max(preco_max)
        receba_ate = tratar_receba_ate(receba_ate)
    elif tipo_afiliacao == "AFILIAÇÃO PEDIDA":
        receba_ate = list_text[list_text.index('enviado!') - 7].replace(".", "").replace(",", ".")
        moeda = list_text[list_text.index('enviado!') - 5]
        preco_max = list_text[list_text.index('enviado!') + 12][:-1].replace(".", "").replace(",", ".")
        moeda = tratar_moeda(moeda)
        preco_max = tratar_preco_max(preco_max)
        receba_ate = tratar_receba_ate(receba_ate)
    else:
...
for i in range(0, len(df_urls)):
    ...
    df_final.to_excel(r"G:\My Drive\Affiliate Marketing\urls_new_courses\2021\3-March\selected_courses_requested_20210306.xlsx")
