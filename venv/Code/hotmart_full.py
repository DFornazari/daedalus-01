...
def get_urls(country):
    ...
    for market in list_market_types:
        url_market = r"https://app-vlc.hotmart.com/market" + market + '&countryId=' + dict_countries[country]['countryId'] + '&locale=' + dict_countries[country]['locale']
    ...
    df_urls.columns = ['URL']
    df_urls.to_excel(f"G:\\My Drive\\Affiliate Marketing\\database_courses\\{str(data.year)}\\{dict_months[data.month]}\\df_urls_{country}_{data_string}.xlsx")
    return df_urls
...
def get_courses_infos(df_urls):
    ...
    for i in range(0, len(df_urls)):
        ...
        df_final.to_excel(f"G:\\My Drive\\Affiliate Marketing\\df_final_{country}_{data}.xlsx")
    ...
    for i in range(0, len(df_cursos['affiliateUrl'])):
        ...
        df_cursos.to_excel(f"G:\\My Drive\\Affiliate Marketing\\cursos_{country}_{data}.xlsx")


get_urls_switch = False
if get_urls_switch == True:
    df_urls = get_urls(country)

input("Is use stored correct?")
if get_urls_switch == False:
    input("Is use stored correct?")
    path = r"G:\My Drive\Affiliate Marketing\database_courses\2020\12-December\Ready\df_url_ready_2020-12-18.xlsx"
    df_urls = pd.read_excel(path)
